...
public class Statistics {
    ...
    static String getSummary() {
        Map<String, int[]> coursesPoints = Map.of(
                "java", DataStore.students.values().stream().mapToInt(Student::getJavaPoints).toArray(),
                "dsa", DataStore.students.values().stream().mapToInt(Student::getDSAPoints).toArray(),
                "databases", DataStore.students.values().stream().mapToInt(Student::getDataBasePoints).toArray(),
                "spring", DataStore.students.values().stream().mapToInt(Student::getSpringPoints).toArray()
        );
    ...
    private static String findLeastPopular(String mostPopular) {
    ...
    private static String findHighestActivity() {
        int max = DataStore.activity.values().stream()
                .filter(elem -> elem != 0)
                .max(Comparator.comparingInt(elem -> elem))
                .orElse(-1);
        String[] array = DataStore.activity.entrySet().stream()
    ...
    private static String findLowestActivity(String highestActivity) {
        if (highestActivity.equals("n/a"))
            return "n/a";

        int min = DataStore.activity.entrySet().stream()
                .filter(elem ->
    ...
    private static String findEasiestCourse(Map<String, Double> averageScore) {
    ...
    private static String findHardestCourse(Map<String, Double> averageScore) {
        return averageScore.entrySet().stream().filter(elem -> elem.getValue() > 0.0)
                .max(Comparator.comparingDouble(Map.Entry::getValue))
                .map(elem -> DataStore.courseFormat(elem.getKey())).orElse("n/a");
    }

    static String getSummaryByCourse(String course) {
        ...
        DataStore.students.entrySet().stream().filter(filter).sorted(comparator).forEach(forEach);
        return String.join("\n", result);
    }
}
